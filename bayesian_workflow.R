...
# library() # Load library for bayes factors
###########################################################################
# 1. Download Gauge Data --------------------------------------------------
###########################################################################

#Load data
edbk <- read.csv("data/edeback-1950-now.csv",
                 sep = ";",
                 header = T)

edbk$Date <- as.Date(edbk$Date)

# split data into separate types
# last 5 years
edbk5 <-  subset(edbk, Date > as.Date("2018-01-01") )

# last 10 years
edbk10 <-  subset(edbk, Date > as.Date("2013-01-01") )

###########################################################################
# 2. Prepare Gauge Data ---------------------------------------------------
###########################################################################
# retrieve annual flooding events for each dataset
edbk.f <- floo(edbk)
edbk5.f <- floo(edbk5)
edbk10.f <- floo(edbk10)
